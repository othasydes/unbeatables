...
GodmodeMatchesConfiguredPriority = function(word)
    for index, category in ipairs(Config.GodmodePriority) do
        ...
        elseif category == "__LOWEST_ENTRY__" then
            -- V20.3: LOWEST ENTRY has its own lazy full-range search.
            -- Never run its expensive stage simulation in the cheap Godmode tail scan.
        ...
        if bucket then
            ...
            if useBinary and #prefix > 0 then
                ...
                if startIndex ~= -1 then
                    ...
                    -- V20.3: each Godmode priority gets its own tail allowance.
                    -- A lower-priority ending encountered earlier alphabetically can no
                    -- longer consume one shared cap and hide a higher-priority ending.
                    ...
                    for i = startIndex, #bucket do
                        ...
                        elseif sortMode == "Godmode" then
                            -- Continue the cheap scan across the whole matching prefix range.
                            -- Keep a SEPARATE bounded allowance for every configured priority,
                            -- so e.g. TING can never fill the allowance before a later EZ word
                            -- such as SOVENEZ is encountered.
