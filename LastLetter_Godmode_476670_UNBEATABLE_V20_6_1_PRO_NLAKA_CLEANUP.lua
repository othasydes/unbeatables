...
env.WordHelperUnbeatable.RefreshProExclusiveWords = function()
    ...
    local function add(word)
        ...
        -- Canonical duplicate check first; normalized identity prevents two
        -- punctuated spellings of the same Pro word being counted twice.
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    local function better(a, b)
        ...
        -- rf / rl / dn / rs / ln / tl / ls are fallback-only ONLY when the
        -- ACTUAL returned prefix is exactly two letters. Longer 3/4-letter
        -- returned prefixes remain completely unaffected.
