...
-- WordHelper V20.7.1 lightweight launcher - register-limit fix - Pro 485,534 punctuation-trap build
...
UpdateStatus("Loading 476,670 Casual + 8,864 Pro delta...", THEME.Warning)
...
-- ============================================================
-- V20.7 PRO 485,534 DICTIONARY DELTA
-- The normal 476,670 Casual dictionary remains untouched.  This embedded
-- delta contains only the 8,864 additional words present in the user's
-- current 485,534-word Pro union, so Pro Unbeatable gets the full Pro pool
...
-- ============================================================
env.WordHelperUnbeatable.ProDictionaryTotal = 485534
...
env.WordHelperUnbeatable.RefreshProExclusiveWords = function()
    ...
    local function add(word)
        ...
        -- V20.7: the 485,534-word Pro union already contains the old hardcoded
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    local function better(a, b)
        ...
        -- it can no longer jump above a valid punctuation trap.
    ...
    for i = 1, math.max(#displayList, #ButtonCache) do
        ...
        if w then
            ...
            elseif sortMode == "Pro Unbeatable" then
                ...
                if pro.ProPriorityTwoLetter then
    ...
    local success, err = pcall(function()
        ...
        if isVisible then
            ...
            elseif typeText ~= env.WordHelperUsedTracker.ActivePromptTypeText then
                -- Type changed and no new starting-prefix could be parsed. Drop the old
                -- guard so a genuinely played self-solve may be committed after its turn.
                env.WordHelperUsedTracker.ActivePrompt = ""
                env.WordHelperUsedTracker.ActivePromptTypeText = typeText
            end

            local barePrompt = activePrompt ~= "" and cleanObserved == activePrompt

            if barePrompt then
                -- Clear a matching prompt candidate so a turn transition cannot
                -- accidentally consume a genuine self-solve word from the dictionary.
