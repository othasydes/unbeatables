...
GodmodeSanitizePriority = function(list)
    ...
    for _, value in ipairs(list) do
        ...
        if item == "__TRAP__" or item == "__X__" then
    ...
    -- Keep both special categories available even if an older/bad config omitted them.
...
-- V20: user-confirmed casual-server 2-letter returns that genuinely trigger.
-- These extend the existing hardcoded TrapWordPriority and CustomTrapWords;
-- they do NOT replace either one.
...
GodmodeIsTrapWord = function(word)
    ...
    -- Do not blindly classify every word ending in a niche pair as a trap.
    -- At Stage 3/4 a longer suffix may return first.  Only promote it through
    -- the hardcoded 2-letter set when the ACTUAL casual returned prefix is
    -- exactly two letters and is one of the confirmed working pairs.
    if env.WordHelperUnbeatable
        and env.WordHelperUnbeatable.GetCandidateInfo then
        local info = env.WordHelperUnbeatable.GetCandidateInfo(word)
        if info
            and info.Qualified
            and #info.Prefix == 2
            and GodmodeConfirmedTwoLetter[info.Prefix] == true then
            return true
        end
    end
...
GodmodeGetPriorityCategory = function(word)
    ...
    for index, category in ipairs(Config.GodmodePriority) do
        ...
        elseif #word >= #category and word:sub(-#category) == category then
            if GodmodeHasAvailableReply(category) then
                return category:upper(), index, false
    ...
    if exhaustedIndex then
        return exhaustedEnding:upper() .. " EXHAUSTED", exhaustedIndex, true
    end
...
local function GetGodmodeScore(word)
    local category, categoryIndex, exhausted = GodmodeGetPriorityCategory(word)

    -- Existing TRAP / spam / X priorities remain absolute top priority.
    if categoryIndex and not exhausted then
        return 10000 - categoryIndex
    end

    -- V20 counter-trap fallback. This function is defined later after
    -- GetCandidateInfo exists, but is available by the time UpdateList runs.
    if GodmodeGetCounterTrapScore then
        local counterScore = GodmodeGetCounterTrapScore(word)
        if counterScore then
            return counterScore
        end
    end

    -- Exhausted strategic endings remain visible only after counter-traps.
...
env.WordHelperGodmodeEditor.Info.Text = "Top = highest priority. Edit endings directly. TRAP/X are special."
...
env.WordHelperGodmodeEditor.Refresh = function()
    ...
    for index, category in ipairs(Config.GodmodePriority) do
        ...
        local special = category == "__TRAP__" or category == "__X__"
        local editor
        if special then
            editor = Instance.new("TextLabel", row)
            editor.Text = category == "__TRAP__" and "TRAP" or "X"
            editor.TextColor3 = category == "__TRAP__" and THEME.Success or THEME.Accent
        else
        ...
        editor.Size = UDim2.new(0, 135, 1, -6)
        ...
        up.Position = UDim2.new(0, 170, 0, 3)
        ...
        down.Position = UDim2.new(0, 212, 0, 3)
...
-- ============================================================
-- V20 GODMODE COUNTER-TRAP FALLBACK
--
-- When no configured Godmode spam/trap priority is available for the current
-- prompt, prefer the legal returned prefix with the smallest opponent pool.
--
-- Casual rule used here:
--   minimum 3 NON-SELF replies.
-- A self-solve does not count toward those three, so a self-solving returned
-- prefix needs at least 4 total usable entries.
--
-- The actual returned prefix is still chosen longest-first according to the
-- current Stage 1/2/3/4 through GetCandidateInfo.
-- ============================================================
GodmodeGetCounterTrapInfo = function(word)
    if not env.WordHelperUnbeatable or not env.WordHelperUnbeatable.GetCandidateInfo then
        return nil
    end

    local info = env.WordHelperUnbeatable.GetCandidateInfo(word)
    if not info or not info.Qualified or info.Prefix == "" then
        return nil
    end

    -- For literal 2-letter returns, only trust the user-confirmed casual set.
    -- 3- and 4-letter returns are preserved normally, even when they happen
    -- to END with some untrusted 2-letter pair.
    if #info.Prefix == 2 and not GodmodeConfirmedTwoLetter[info.Prefix] then
        return nil
    end

    local nonSelfReplies = info.Replies - (info.SelfSolve and 1 or 0)
    if nonSelfReplies < 3 then
        return nil
    end
    ...
    return {
        ...
        TotalReplies = info.Replies,
        SelfSolve = info.SelfSolve,
        Stage = info.Stage,
        Turn = info.Turn
    }
end

GodmodeGetCounterTrapScore = function(word)
    local info = GodmodeGetCounterTrapInfo(word)
    if not info then return nil end

    -- Always below configured Godmode categories (~9990+), always above ordinary
    -- zero-score words.  Fewer non-self replies is stronger; for the same pool,
    -- prefer the longer returned prefix.
    local clamped = math.min(info.NonSelfReplies, 1000)
    return 5000 + (1000 - clamped) * 4 + #info.Prefix
end

-- Scan the complete matching prefix range ONLY when there is no live configured
-- Godmode priority. This keeps normal spam turns fast while still finding an
-- obscure low-pool counter-trap when an opponent breaks out of the spam.
GodmodeAugmentWithCounterTraps = function(exacts, bucket, prefix, tryFallbackLengths)
    if type(exacts) ~= "table" or type(bucket) ~= "table" then
        return exacts
    end

    -- If TRAP / ting / ler / etc. is currently available, keep the normal fast
    -- Godmode strategy and do not pay for a full counter-trap scan.
    for _, existing in ipairs(exacts) do
        local _, categoryIndex, exhausted = GodmodeGetPriorityCategory(existing)
        if categoryIndex and not exhausted then
            return exacts
        end
    end
    ...
    local function betterScore(scoreA, wordA, scoreB, wordB)
    ...
    local function insertCandidate(word, score)
        if #bestWords < keepLimit then
            bestWords[#bestWords + 1] = word
            bestScores[word] = score
            return
        end

        local worstIndex = 1
        local worstWord = bestWords[1]
        local worstScore = bestScores[worstWord] or -math.huge

        for i = 2, #bestWords do
            local existingWord = bestWords[i]
            local existingScore = bestScores[existingWord] or -math.huge
            if betterScore(worstScore, worstWord, existingScore, existingWord) then
                worstIndex = i
                worstWord = existingWord
                worstScore = existingScore
            end
        end

        if betterScore(score, word, worstScore, worstWord) then
            bestScores[worstWord] = nil
            bestWords[worstIndex] = word
            bestScores[word] = score
        end
    end
    ...
    for i = startIndex, #bucket do
        ...
        if not Blacklist[word]
            ...
            local score = GodmodeGetCounterTrapScore(word)
            if score then
                insertCandidate(word, score)
            end
        end
    end
    ...
    for _, word in ipairs(bestWords) do
        if not seen[word] then
            exacts[#exacts + 1] = word
            seen[word] = true
        end
    end
    ...
    local function CollectMatches(prefix, tryFallbackLengths)
        ...
        if bucket then
            ...
            if sortMode == "Godmode" then
                exacts = GodmodeAugmentWithCounterTraps(
                    exacts,
                    bucket,
                    prefix,
                    tryFallbackLengths
    ...
    for i = 1, math.max(#displayList, #ButtonCache) do
        ...
        if w then
            ...
            if sortMode == "Godmode" then
                ...
                categoryTag = select(1, GodmodeGetPriorityCategory(w))
                ...
                if categoryTag ~= "" then
                    ...
                    local counter = GodmodeGetCounterTrapInfo(w)
                    if counter then
                        displayText = displayText
                            .. "  <font color=\"rgb(150,150,160)\">[COUNTER -> "
                            .. counter.Prefix:upper()
                            .. " / "
                            .. tostring(counter.NonSelfReplies)
                            .. " NONSELF]</font>"
                    end
                end
            elseif sortMode == "Unbeatable" then
