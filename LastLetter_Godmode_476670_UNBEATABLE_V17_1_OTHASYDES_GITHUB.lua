...
env.WordHelperUnbeatable.GetBaseReplyInfo = function(prefix)
    ...
    local prefixVersion = (env.WordHelperUnbeatable.PrefixVersion[prefix] or 0)
    local cacheKey = tostring(prefix) .. "|v" .. tostring(prefixVersion)
    local cached = env.WordHelperUnbeatable.ReplyCache[cacheKey]
    if cached then return cached.Count, cached.SelfSolve end

    local count = 0
    local selfSolve = false

    -- All Unbeatable return prefixes are length 1-4, so jump straight to the
    -- precomputed contiguous range instead of searching/scanning a whole bucket.
    local range = PrefixRanges[prefix]
    if range then
        for i = range[1], range[2] do
            local reply = Words[i]
            if not Blacklist[reply] and not UsedWords[reply] then
                count = count + 1
                if reply == prefix then selfSolve = true end
            end
        end
    end

    env.WordHelperUnbeatable.ReplyCache[cacheKey] = {
        Count = count,
        SelfSolve = selfSolve
    }
...
UpdateList = function(detectedText, requiredLetter)
            ...
            if useBinary and #prefix > 0 then
                ...
                if startIndex ~= -1 then
                    ...
                    for i = startIndex, #bucket do
                        ...
                        elseif sortMode ~= "Godmode" and sortMode ~= "Unbeatable" then
                            break
                        end
                    end
                end
    ...
    if #matches > 0 then
        ...
        elseif sortMode == "Unbeatable" then
            ...
            -- Do NOT wipe the caches on every list refresh. They are invalidated
            -- only when used-word state/stage/reset changes, which avoids rescanning
            -- the 476k dictionary during harmless UI refreshes.
            for _, w in ipairs(matches) do
                local info = env.WordHelperUnbeatable.GetCandidateInfo(w)
                -- Convenience mirror for the existing sort/display code.
                env.WordHelperUnbeatable.SortCache[w] = info
            end

            -- V11: confirmed 2-letter traps are PRIORITY, not a hard filter.
            -- Keep every valid suggestion in the pool. The existing score gives
            -- confirmed KnownTwoLetter endings a strong advantage, while generic
            -- low-entry 2-letter prefixes remain visible underneath as fallback.

            table.sort(matches, function(a, b)
                local infoA = env.WordHelperUnbeatable.SortCache[a]
                local infoB = env.WordHelperUnbeatable.SortCache[b]

                -- V16 Stage-2 priority:
                --   1) confirmed working 2-letter traps
                --   2) all other non-suppressed candidates
                --   3) confirmed non-triggering 2-letter prefixes as fallback only
                -- Latest live-test suppression overrides any older KnownTwoLetter entry.
                if (env.WordHelperUnbeatable.Stage or 1) == 2 then
