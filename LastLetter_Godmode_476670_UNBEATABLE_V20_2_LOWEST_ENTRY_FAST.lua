...
GodmodeSanitizePriority = function(list)
    ...
    -- Keep all hardcoded special categories available even when loading an older
    -- saved config that predates LOWEST ENTRY.
...
-- V20.1: confirmed casual-server 2-letter returned prefixes.
-- These are trusted ONLY when the ACTUAL returned prefix is exactly 2 letters.
-- Longer 3/4-letter returns are never cancelled just because they end in one
-- of these pairs.
GodmodeConfirmedTwoLetter = {
    ["zi"] = true, ["sd"] = true, ["md"] = true, ["mg"] = true,
    ["rg"] = true, ["yg"] = true, ["yp"] = true, ["yc"] = true,
    ["mh"] = true, ["nh"] = true, ["dh"] = true, ["kh"] = true,
    ["dz"] = true, ["lw"] = true, ["dw"] = true, ["sf"] = true,
    ["sv"] = true, ["tz"] = true, ["tx"] = true, ["kt"] = true,
    ["nk"] = true, ["hl"] = true, ["yw"] = true, ["bt"] = true,
    ["mb"] = true, ["fd"] = true, ["pk"] = true, ["sz"] = true,
    ["hd"] = true, ["pf"] = true, ["cw"] = true, ["kw"] = true,
    ["sb"] = true, ["sg"] = true
}

GodmodeIsTrapWord = function(word)
    if TrapWordPriority[word] == true or CustomTrapWords[word] == true then
        return true
    end

    -- Confirmed casual 2-letter trap returns are handled stage-aware by
    -- LOWEST ENTRY / Unbeatable logic. Avoid running expensive simulation here.
    return false
end
...
GodmodeMatchesConfiguredPriority = function(word)
    for _, category in ipairs(Config.GodmodePriority) do
        if category == "__TRAP__" then
            if GodmodeIsTrapWord(word) then return true end
        elseif category == "__LOWEST_ENTRY__" then
            if GodmodeGetLowestEntryInfo and GodmodeGetLowestEntryInfo(word) then return true end
        elseif category == "__X__" then
            if word:sub(-1) == "x" then return true end
...
GodmodeGetPriorityCategory = function(word)
    ...
    for index, category in ipairs(Config.GodmodePriority) do
        ...
        elseif category == "__LOWEST_ENTRY__" then
            -- V20.2 FAST: only words placed in the current LOWEST ENTRY shortlist
            -- qualify here. Do NOT run stage simulation from the comparator.
...
local function GetGodmodeScore(word)
    ...
    if categoryIndex and not exhausted then
        if category == "LOWEST ENTRY" and lowestInfo then
            -- Stay inside this priority slot while ranking the nastiest pool first.
            -- 3 non-self replies is ideal; then 4, 5, 6...
            local poolBonus = 0.90 / math.max(1, lowestInfo.NonSelfReplies - 2)
            local lengthBonus = math.min(#lowestInfo.Prefix, 4) * 0.001
            return (10000 - categoryIndex) + poolBonus + lengthBonus
        end
...
env.WordHelperGodmodeEditor.Info.Text = "Top = highest priority. TRAP / LOWEST ENTRY / X are hardcoded movable priorities."
...
env.WordHelperGodmodeEditor.Refresh = function()
    ...
    for index, category in ipairs(Config.GodmodePriority) do
        ...
        editor.Size = UDim2.new(0, 150, 1, -6)
        ...
        up.Position = UDim2.new(0, 182, 0, 3)
        ...
        down.Position = UDim2.new(0, 224, 0, 3)
...
env.WordHelperUnbeatable.KnownTwoLetterTraps = {
    ...
    mh=true, nh=true, dh=true, kh=true, dz=true, lw=true, dw=true, sv=true, tz=true,
    tx=true, kt=true, nk=true, hl=true, yw=true, bt=true, mb=true, fd=true, pk=true,
    sz=true, hd=true, pf=true, cw=true, kw=true, sb=true
}
...
-- ============================================================
-- V20.1 TRUE GODMODE "LOWEST ENTRY"
--
-- This is an actual Godmode priority category, not an invisible fallback.
-- For each candidate it simulates the prefix the opponent would REALLY receive
-- at the current stage, longest suffix first.
--
-- Requirement requested by user:
--   at least 3 usable responses EXCLUDING the prefix's own self-solve.
--
-- Hidden casual 2-letter behavior:
--   literal 2-letter returns are trusted only if present in the user's confirmed
--   GodmodeConfirmedTwoLetter set.  If a 2-letter suffix is unconfirmed, we keep
--   falling back to the 1-letter suffix instead of cancelling the whole word.
--
-- 3/4-letter preservation:
--   no 2-letter suppression is applied to a 3- or 4-letter returned prefix.
-- ============================================================
GodmodeLowestEntryCache = GodmodeLowestEntryCache or {}
GodmodeLowestEntryShortlistCache = GodmodeLowestEntryShortlistCache or {}
...
GodmodeGetLowestEntryInfo = function(candidate)
    ...
    -- Prefix-version-aware cache: only suffix pools that could affect this word
    -- invalidate its result.
    local versionParts = {}
    for suffixLen = 1, math.min(stage, #candidate) do
        local p = candidate:sub(-suffixLen)
        versionParts[#versionParts + 1] =
            p .. ":" .. tostring(env.WordHelperUnbeatable.PrefixVersion[p] or 0)
    end
    local cacheKey = candidate .. "|s" .. tostring(stage) .. "|" .. table.concat(versionParts, ",")
    local cached = GodmodeLowestEntryCache[cacheKey]
    if cached ~= nil then
    ...
    for suffixLen = math.min(stage, #candidate), 1, -1 do
        ...
        -- Casual server's hidden 2-letter eligibility layer.
        local trustedLength = (suffixLen ~= 2) or GodmodeConfirmedTwoLetter[prefix]

        if trustedLength then
            local baseCount, baseSelf = env.WordHelperUnbeatable.GetBaseReplyInfo(prefix)

            -- Candidate is about to become used, so remove it if the reply pool
            -- currently counts it as a possible answer.
            local candidateWasCounted =
                candidate:sub(1, #prefix) == prefix
                and not Blacklist[candidate]
                and not UsedWords[candidate]
    ...
    GodmodeLowestEntryCache[cacheKey] = chosen or false
    return chosen
end

-- Pull the strongest LOWEST ENTRY candidates from the COMPLETE current prefix
-- range, rather than only the first 1,400 alphabetic words. This is what allows
-- obscure replies such as a niche 3/4-letter counter-trap to be discovered.
GodmodeAugmentLowestEntryCandidates = function(exacts, bucket, prefix, tryFallbackLengths)
    if type(exacts) ~= "table" or type(bucket) ~= "table" then return exacts end

    -- Clear previous active LOWEST ENTRY set every refresh.
    table.clear(GodmodeLowestEntryActiveSet)
    table.clear(GodmodeLowestEntryActiveInfo)

    if not prefix or prefix == "" or prefix:find("#") or prefix:find("%*") then
        return exacts
    end
    ...
    -- V20.2 LAZY GATE:
    -- If any CURRENT exact result satisfies a live category ABOVE LOWEST ENTRY,
    -- do not do counter-trap work at all.
    ...
    -- Cache a broad shortlist by current typed prefix + stage. The expensive full
    -- dictionary-range scan is done only the first time this prompt/stage is seen.
    env.WordHelperUnbeatable.SyncStageFromUsedWords()
    local stage = math.clamp(tonumber(env.WordHelperUnbeatable.Stage) or 1, 1, 4)
    local shortlistKey = tostring(stage) .. "|" .. prefix
    local shortlist = GodmodeLowestEntryShortlistCache[shortlistKey]

    if not shortlist then
        shortlist = {}
        local staticKeep = 320

        local function staticInfoFor(word)
            local best = nil
            for suffixLen = math.min(stage, #word), 1, -1 do
                local p = word:sub(-suffixLen)

                -- Exact 2-letter returns require confirmed casual-server support.
                if suffixLen ~= 2 or GodmodeConfirmedTwoLetter[p] then
                    local range = env.WordHelperUnbeatable.PrefixRanges[p]
                    if range then
                        local total = range[2] - range[1] + 1
                        local selfSolve = WordHelperKnownWords[p] == true
                        local nonSelf = total - (selfSolve and 1 or 0)

                        -- Candidate itself would become used if it starts with p.
                        if word:sub(1, #p) == p then
                            nonSelf = nonSelf - 1
                        end
        ...
        for i = startIndex, #bucket do
            ...
            if not Blacklist[word]
                ...
                local info = staticInfoFor(word)
                if info then
                    local entry = {Word = word, Info = info}
                    if #shortlist < staticKeep then
                        shortlist[#shortlist + 1] = entry
                    else
                        local worstIndex = 1
                        for j = 2, #shortlist do
                            if better(shortlist[worstIndex], shortlist[j]) then
                                worstIndex = j
                            end
                        end
                        if better(entry, shortlist[worstIndex]) then
                            shortlist[worstIndex] = entry
                        end
                    end
                end
            end
        ...
        GodmodeLowestEntryShortlistCache[shortlistKey] = shortlist
    end

    -- Dynamic pass: only re-evaluate the cached shortlist against current
    -- UsedWords / Blacklist counters. This preserves trap exhaustion behavior.
    local dynamic = {}
    local keepLimit = 120

    local function dynamicBetter(a, b)
        if not b then return true end
        if a.Info.NonSelfReplies ~= b.Info.NonSelfReplies then
            return a.Info.NonSelfReplies < b.Info.NonSelfReplies
        end
    ...
    for _, entry in ipairs(shortlist) do
        ...
        if not Blacklist[word] and (not UsedWords[word] or Config.ShowUsedWords) then
            local info = GodmodeGetLowestEntryInfo(word)
            if info then
                local wrapped = {Word = word, Info = info}
                if #dynamic < keepLimit then
                    dynamic[#dynamic + 1] = wrapped
                else
                    local worstIndex = 1
                    for j = 2, #dynamic do
                        if dynamicBetter(dynamic[worstIndex], dynamic[j]) then
                            worstIndex = j
                        end
                    end
                    if dynamicBetter(wrapped, dynamic[worstIndex]) then
                        dynamic[worstIndex] = wrapped
                    end
                end
            end
        end
    ...
    table.sort(dynamic, function(a, b) return dynamicBetter(a, b) end)
    ...
    local function CollectMatches(prefix, tryFallbackLengths)
        ...
        if bucket then
                ...
                if startIndex ~= -1 then
                    ...
                    for i = startIndex, #bucket do
                        ...
                        elseif sortMode == "Godmode" then
                            -- Continue the cheap scan so late priority matches are not lost,
                            -- but bound non-trap additions to avoid another UI freeze.
                            local canAddPriority = GodmodeMatchesConfiguredPriority(w)
                            local isTrapPriority = GodmodeIsTrapWord(w)
                            if canAddPriority
                                and (isTrapPriority or tailPriorityAdded < 350)
                                and not seenExact[w]
                                and not Blacklist[w]
                                ...
                                table.insert(exacts, w)
                                seenExact[w] = true
                                if not isTrapPriority then
                                    tailPriorityAdded = tailPriorityAdded + 1
                                end
                            end
            ...
            if sortMode == "Godmode" then
                exacts = GodmodeAugmentLowestEntryCandidates(
                    exacts,
                    bucket,
                    prefix,
                    tryFallbackLengths
