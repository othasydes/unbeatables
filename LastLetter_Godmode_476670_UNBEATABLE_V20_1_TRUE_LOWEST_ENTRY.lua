...
GodmodeIsTrapWord = function(word)
    ...
    -- Extend the existing hardcoded/custom trap list with the user's confirmed
    -- casual 2-letter trap returns.  The stage-aware helper is defined later;
    -- by the time the UI is used it is available.
    if GodmodeGetLowestEntryInfo then
        local info = GodmodeGetLowestEntryInfo(word)
        if info and #info.Prefix == 2 and GodmodeConfirmedTwoLetter[info.Prefix] then
            return true
        end
    end
...
GodmodeGetPriorityCategory = function(word)
    ...
    for index, category in ipairs(Config.GodmodePriority) do
        ...
        elseif category == "__LOWEST_ENTRY__" then
            if GodmodeGetLowestEntryInfo then
                lowestInfo = lowestInfo or GodmodeGetLowestEntryInfo(word)
                if lowestInfo then
                    return "LOWEST ENTRY", index, false, lowestInfo
                end
            end
...
GodmodeAugmentLowestEntryCandidates = function(exacts, bucket, prefix, tryFallbackLengths)
    ...
    local function addBest(word, info)
        if seenBest[word] then return end
        seenBest[word] = true
        ...
        local worstIndex = 1
        for i = 2, #best do
            if isBetter(best[worstIndex], best[i]) then
                worstIndex = i
            end
        end

        if isBetter(entry, best[worstIndex]) then
            seenBest[best[worstIndex].Word] = nil
