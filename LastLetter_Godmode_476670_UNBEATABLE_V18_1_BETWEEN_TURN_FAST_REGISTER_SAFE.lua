...
-- V18 FAST: number of unavailable replies per 1-4 letter starting prefix.
-- PrefixRanges already tells us the total number of dictionary words beginning
-- with a prefix.  Keeping this tiny dynamic counter means a reply-count lookup is
-- O(1) even immediately after a used word changes, instead of rescanning every
-- word under broad prefixes such as S/A/C/P between turns.
...
env.WordHelperUnbeatable.RebuildUnavailablePrefixCount = function()
    ...
    -- Union of Blacklist and UsedWords: never subtract the same word twice.
...
-- V18.1: exact top-K Unbeatable selector in its own function/register frame.
-- Every legal candidate is still evaluated, so rare traps are preserved, while
-- only the best 120 are retained for the final sort/render.
env.WordHelperUnbeatable.SelectTopCandidates = function(candidateList)
    ...
    -- Root of this min-heap is the worst retained candidate.
...
env.WordHelperUsedTracker.RecordObservedUsedWord = function(word)
    ...
    UsedWords[word] = true
    -- V18 FAST: update O(1) reply availability counters only when this word was
    -- not already unavailable through the blacklist.
    if env.WordHelperUnbeatable
        and env.WordHelperUnbeatable.AdjustUnavailableWord
